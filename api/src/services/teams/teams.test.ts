...
describe('teams', () => {
  ...
  scenario('creates a team', async (scenario: StandardScenario) => {
    const result = await createTeam({
      input: { name: 'String', seasonId: scenario.team.two.seasonId },
    })

    expect(result.name).toEqual('String')
    expect(result.seasonId).toEqual(scenario.team.two.seasonId)
  })
